...
class PlayState extends FlxState
{
	...
	{
		trace('State camera: ${Global.width} x ${Global.height}');

		// The grass background
		var bgSize:Int = 32;
		var bgWidth:Int = Math.ceil(Global.width / bgSize) * bgSize;
		var bgHeight:Int = Math.ceil(Global.height / bgSize) * bgSize;
		...
		else
		{
			...
			add(bg.loadTiles(Global.asset("assets/grass.png")));
		}
	...
	function createOverlay():FlxSpriteGroup
	{
		...
		uiBackground.makeGraphic(Global.width, 100, FlxColor.WHITE);
		...
		// Right UI
		// Column2 1
		var rightButtonX:Float = Global.width - 100;
		...
		// The texts
		_bunnyCounter = new FlxText(0, 10, Global.width, "Bunnies: " + _bunnies.length);
		_bunnyCounter.setFormat(null, 22, FlxColor.BLACK, CENTER);
		overlay.add(_bunnyCounter);

		_fpsCounter = new FlxText(0, _bunnyCounter.y + _bunnyCounter.height + 20, Global.width, "FPS: " + 30);
